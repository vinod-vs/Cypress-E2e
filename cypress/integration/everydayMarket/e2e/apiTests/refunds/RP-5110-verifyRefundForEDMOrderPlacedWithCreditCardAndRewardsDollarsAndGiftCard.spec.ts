    ...
    it('RP-5110 - EM | Refunds | Verify refunds happens to the right payment mode for market orders placed via CC + RD + GC', () => {
      ...
      cy.get('@placedMarketOrderApiData').then((data: any) => {
          ...
          cy.orderEventsApiWithRetry(data.orderReference, {
            function: function (response: any) {
              if (
                ...
                cy.log(
                  'Expected MarketOrderRefund and RefundCompleted events to be present'
                )
                throw new Error(
                  'Expected MarketOrderRefund and RefundCompleted events to be present'
                )
        ...
        cy.get('@refundPaymentsDetails').then((refundPaymentsDetails: any) => {
          ...
          cy.findCCRefundPayment(
            refundPaymentsDetails,
            data.invoices[0].invoiceTotal +
              data.shippingAmount -
              0.01 -
              rewardsDollarsToRedeem
          ).then((isFound: any) => {
            cy.log(
              'Expected credit card refund: ' +
                (
                  data.invoices[0].invoiceTotal +
                  data.shippingAmount -
                  0.01 -
          ...
          cy.findSCRefundPayment(refundPaymentsDetails, 0.01).then(
            (isFound: any) => {
              expect(isFound).to.be.true
            }
          )
