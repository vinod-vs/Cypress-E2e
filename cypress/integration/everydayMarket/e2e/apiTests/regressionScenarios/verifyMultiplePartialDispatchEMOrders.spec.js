  ...
  describe('[API] RP-5040 - Multiple Partial Dispatch Everyday Market order via Marketplacer', () => {
    ...
    it('RP-5040 - Multiple Partial Dispatch Everyday Market order via Marketplacer', () => {
      ...
      cy.loginAndPlaceRequiredOrderFromTestdata(
        shoppers.emAccountWithRewards22,
        testData
      ).then((response) => {
        ...
        cy.ordersApiByShopperIdAndTraderOrderIdWithRetry(shopperId, orderId, {
          ...
          cy.partialDispatchOfLineItemsInInvoice(
            ...
            verifyOrderProjectionDetails(
              shopperId,
              orderId,
              testData,
              trackingId1,
              orderReference,
              partialDispatchNumber
            )
          })
  ...
  cy.ordersApiByShopperIdAndTraderOrderIdWithRetry(shopperId, orderId, {
    ...
    .then((response) => {
      ...
      if (partialDispatchNumber === 1) {
        ...
        expect(response.invoices[0].shipments.length).to.be.equal(1)
        expect(response.invoices[0].shipments[0].trackingNumber).to.be.equal(
          trackingId
        )
        ...
        // Shipments
        expect(
          response.invoices[0].shipments[1].shippedItems[0].quantity
        ).to.be.equal(1)
        expect(response.invoices[0].shipments.length).to.be.equal(2)
        expect(response.invoices[0].shipments[1].trackingNumber).to.be.equal(
          trackingId
        )
        expect(response.invoices[0].shipments[1].carrier).to.be.equal(
          testData.carrier
        )
        ...
        expect(
          response.invoices[0].shipments[1].shippedItems.length
        ).to.be.equal(1)
        expect(
          response.invoices[0].shipments[1].shippedItems[0].variantId
        ).to.be.equal(response.invoices[0].lineItems[0].variantId)
        expect(
          response.invoices[0].shipments[1].shippedItems[0].stockCode
        ).to.be.equal(Number(testData.items[0].stockCode))
      }
      ...
      expect(response.invoices[0].lineItems[0].reward.deferredDiscountAmount).to
        .not.be.null
      ...
      cy.orderEventsApiWithRetry(orderReference, {
        function: function (response) {
          if (partialDispatchNumber === 1) {
            if (
              !response.body.data.some(
                (element) => element.domainEvent === 'MarketOrderShipmentCreate'
              ) ||
              !response.body.data.some(
      ...
      if (Cypress.env('marketRewardPointsValidationSwitch')) {
        ...
        cy.getRewardsCardDetails(
          ...
          const expectedRewardsPoints = Math.floor(
            Number(testData.edmTotal) + Number(testData.rewardPointBefore)
          )
          ...
          cy.log(
            'Expected New Rewards Balance: ' +
              Math.floor(expectedRewardsPoints) +
              ' , OR: ' +
              Number(Number(Math.round(expectedRewardsPoints + 1)))
          )
          expect(response.queryCardDetailsResp.pointBalance).to.be.greaterThan(
            0
          )
          ...
          if (partialDispatchNumber === 1) {
            const expectedRewardsPtsPartialDispatch = Math.floor(
              expectedRewardsPoints - testData.items[0].pricePerItem
            )
            expect(response.queryCardDetailsResp.pointBalance).to.be.gte(
              expectedRewardsPtsPartialDispatch
            )
          } else {
            expect(response.queryCardDetailsResp.pointBalance).to.be.gte(
