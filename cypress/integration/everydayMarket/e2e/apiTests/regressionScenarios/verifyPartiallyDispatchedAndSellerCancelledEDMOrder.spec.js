    ...
    it('RP-5044 - Partial Dispatch and Partial seller cancellation (partial OOS) Everyday Market order. {Failing because of BUG: MPPF-1450}', () => {
      ...
      cy.get('@confirmedTraderOrder').then((confirmedOrder) => {
        ...
        if (Cypress.env('marketRewardPointsValidationSwitch')) {
          ...
          cy.getRewardsCardDetails(
            ...
            rewards.siteId,
            rewards.posId,
            rewards.loyaltySiteType,
            shopper.rewardsCardNumber
          )
            .its('queryCardDetailsResp')
            .its('pointBalance')
            .as('rewardBalanceBefore')
        }
      })
      ...
      cy.get('@placedMarketOrderApiData').then((data) => {
        ...
        cy.cancelLineItemInInvoice(
          data.invoices[0].invoiceId,
          data.invoices[0].lineItems[0].lineItemId,
          cancelledQty,
          false
        ).then(() => {
          cy.orderEventsApiWithRetry(req.orderReference, {
            function: function (response) {
              if (
                ...
                cy.log(
                  'Expected RefundRequestUpdate, MarketOrderRefund & RefundCompleted to be present'
                )
                throw new Error(
                  'Expected RefundRequestUpdate, MarketOrderRefund & RefundCompleted to be present'
                )
          ...
          cy.ordersApiByShopperIdAndTraderOrderIdWithRetry(
            ...
            .then((response) => {
              ...
              // Validate refunds
              expect(response.invoices[0].refunds[0].refundAmount).is.equal(
                cancelledQty * data.invoices[0].lineItems[0].salePrice
              )
              expect(response.invoices[0].refunds[0].totalAmount).is.equal(
                cancelledQty * data.invoices[0].lineItems[0].salePrice
              )
              ...
              ).is.equal(sellerCancellation.reason)
              expect(
              ...
              ).is.equal(cancelledQty * data.invoices[0].lineItems[0].salePrice)
              ...
              ).is.equal(data.invoices[0].lineItems[0].quantity)
              expect(
                response.invoices[0].refunds[0].refundItems[0].lineItem
                  .salePrice
              ).is.equal(data.invoices[0].lineItems[0].salePrice)
              // Validate shipments
              expect(response.invoices[0].shipments[0].carrier).is.equal(
                shipment.postageCarrier
              )
          ...
          if (Cypress.env('marketRewardPointsValidationSwitch')) {
            ...
            cy.all(
              ...
              const expectedRewards = Math.floor(
                Number(before) +
                  Number(
                    cancelledOrderProjection.invoices[0].refunds[0]
                      .refundItems[0].amount
                  )
              )
