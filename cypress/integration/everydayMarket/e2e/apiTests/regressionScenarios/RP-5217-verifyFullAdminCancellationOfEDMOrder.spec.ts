...
TestFilter(['EDM', 'API', 'EDM-E2E-API', 'E2E-Scenario-1'], () => {
  describe('[API] RP-5217 - EM | Customer gets refund for the items fully cancelled by ADMIN', () => {
    before(() => {
      cy.clearCookies({ domain: null })
      cy.clearLocalStorage({ domain: null })
    })

    it('[API] RP-5217 - EM | Customer gets refund for the items fully cancelled by ADMIN', () => {
      const purchaseQty = 2
      ...
      cy.get('@confirmedTraderOrder').then((confirmedOrder: any) => {
        ...
        // Call Market Order Events API and validate the data before order being fully admin cancelled
        ...
        // Call Market Order API and validate the data before order being fully admin cancelled
        cy.ordersApiByShopperIdAndTraderOrderIdWithRetry(
          ...
          // Perform partial admin cancellation on the EDM order (2 out of 2 items)
        ...
        // Call Market Order Events API and validate the data after order being fully admin cancelled
        cy.orderEventsApiWithRetry(req.orderReference, {
          function: function (response: any) {
            if (
              !response.body.data.some(
        ...
        // Call Market Order API and validate the data after order being fully admin cancelled
        cy.ordersApiByShopperIdAndTraderOrderIdWithRetry(
          req.shopperId,
          req.orderId,
          {
