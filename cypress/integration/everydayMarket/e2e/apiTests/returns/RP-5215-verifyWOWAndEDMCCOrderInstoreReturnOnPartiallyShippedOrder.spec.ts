...
import { verifyOrderTotals, verifyInitialOrderDetails, verifyEventDetails, verifyRefundDetails, verifyOQSOrderStatus } from '../../../../../support/everydayMarket/api/commands/commonHelpers'
  ...
  describe('[API] RP-5215 - Instore Return On Partially Shipped Order', () => {
    ...
    it('RP-5215 - Instore Return On Partially Shipped Order', () => {
      ...
      cy.loginAndPlaceRequiredOrderFromTestdata(
        shoppers.emAccountWithRewards8,
        testData
      ).then((response) => {
        ...
        cy.ordersApiByShopperIdAndTraderOrderIdWithRetry(shopperId, orderId, {
          ...
          cy.partialDispatchOfLineItemsInInvoice(
            ...
            [{ line_item_id: lineItemLegacyId, quantity: dispatchQty }],
            ...
            ).to.be.equal(shipment.postageTrackingnumber)
            ...
            cy.ordersApiByShopperIdAndTraderOrderIdWithRetry(
              ...
              testData.orderId,
              {
                function: function (response) {
                  if (
                    response.body.invoices[0].invoiceStatus !== 'PARTIALLY_SENT'
                  ) {
              ...
              ).is.equal(testData.items[0].stockCode)
              expect(
                ...
                cy.ordersApiByShopperIdAndTraderOrderIdWithRetry(
                  ...
                  cy.ordersApiByShopperIdAndTraderOrderIdWithRetry(
                    ...
                    .then((response) => {
                      ...
                      expect(
                        response.invoices[0].refunds[0].status
                      ).to.be.equal('Returned')
                      expect(
                      ...
                      if (Cypress.env('marketRewardPointsValidationSwitch')) {
                        ...
                        cy.getRewardsCardDetails(
                          ...
                          const totalDispatchAmount =
                            Math.floor(Number(testData.edmTotal)) -
                            Math.floor(
                              Number(
                                testData.items[0].pricePerItem * dispatchQty
                              )
                            )
                          cy.log('totalDispatchAmount: ' + totalDispatchAmount)
                          const expectedRewardsPoints =
                            totalDispatchAmount +
                          ...
                          expect(
                            response.queryCardDetailsResp.pointBalance
                          ).to.be.gte(expectedRewardsPoints)
                        })
                      }
