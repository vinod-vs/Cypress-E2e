  ...
  describe('[API] RP-5218  Create Admin isntore return on full Everyday Market items invoice ', () => {
    ...
    it('RP-5218 - Create Admin isntore return on full Everyday Market items invoice ', () => {
      ...
      cy.loginAndPlaceRequiredOrderFromTestdata(
        shoppers.emAccountWithRewards7,
        testData
      ).then((response) => {
              ...
              const returnRequestLineItem = [
                {
                  ...
                  quantity: testData.items[0].quantity,
                ...
                cy.ordersApiByShopperIdAndTraderOrderIdWithRetry(
                  ...
                  cy.ordersApiByShopperIdAndTraderOrderIdWithRetry(
                    ...
                    .then((response) => {
                      ...
                      expect(response.invoices[0].refunds[0].notes[0].id).to.not
                        .be.null
                      expect(
                      ...
                      expect(response.invoices[0].refunds[0].notes[0].timestamp)
                        .to.not.be.null
                      expect(response.invoices[0].refunds[0].notes[1].id).to.not
                        .be.null
                      expect(
                      ...
                      expect(response.invoices[0].refunds[0].notes[1].timestamp)
                        .to.not.be.null
                      expect(response.invoices[0].refunds[0].notes[2].id).to.not
                        .be.null
                      expect(
                        response.invoices[0].refunds[0].notes[2].note
                      ).to.be.equal('Auto-refund cancellation')
                      expect(response.invoices[0].refunds[0].notes[2].timestamp)
                        .to.not.be.null
                      expect(response.invoices[0].returns.length).to.be.equal(0)
                      // Verify the MP and shipping invoices are available for the customer
                      cy.verifyOrderInvoice(testData)
