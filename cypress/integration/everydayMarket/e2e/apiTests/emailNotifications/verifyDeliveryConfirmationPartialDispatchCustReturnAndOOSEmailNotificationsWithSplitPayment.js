...
const testData = tests.VerifyEmailNotifications
let orderId
let orderReference
let edmOrderId
let edmInvoiceId
const shopper = shoppers.emailTest1
const shopperId = shopper.shopperId
const emailId = shopper.email
    ...
    describe('[API] RP-5568 - Email Notifications | Verify delivery confirmation, partial dispatch, customer return-refund and partial OOS email notifications with split payments', () => {
        ...
        it('[API] RP-5568 - Email Notifications | Verify delivery confirmation, partial dispatch, customer return-refund and partial OOS email notifications with split payments', () => {
            ...
            cy.verifyDeliveryConfirmationEmailDetails(testData).then((deliveryConfirmationEmail) => {
                ...
                let trackingId1 = lib.generateRandomString()
        ...
        cy.get('@customerReturnEvents').then((customerReturnEvents) => {
            cy.getOrderStatus(testData.orderId).as('customerReturnOqsResponse').then((customerReturnOqsResponse) => {
                ...
                cy.get('@emailDetails').then((emailDetails) => {
                    ...
                    //Refund amount
                    let refundText = 'Your credit/debit card used for payment will be refunded with the amount of $' + customerReturnOqsResponse.MarketOrders[0].Products[0].SalePrice + '. Your refund may take 3 to 5 business days depending on your financial institution’s processing time.'
        ...
        cy.get('@oosEvents').then((oosEvents) => {
            cy.getOrderStatus(testData.orderId).as('oosOqsResponse').then((oosOqsResponse) => {
                ...
                cy.get('@emailDetails').then((emailDetails) => {
                    ...
                    //Refund amount
                    let oosRefundText = 'Your credit/debit card used for payment will be refunded with the amount of $' + oosOqsResponse.MarketOrders[0].Products[0].SalePrice + '. Your refund may take 3 to 5 business days depending on your financial institution’s processing time.'
