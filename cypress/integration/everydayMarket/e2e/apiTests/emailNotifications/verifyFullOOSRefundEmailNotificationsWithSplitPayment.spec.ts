...
let sentFrom: string = 'shoponline@woolworths.com.au';
    ...
    cy.get('@oosProjection').then((oosProjection) => {
        cy.get('@oosEvents').then((oosEvents) => {
            cy.getOrderStatus(testData.orderId).as('oosOqsResponse').then((oosOqsResponse) => {
                ...
                let expectedSubject = 'Everyday Market order ' + edmOrderId + ' has been cancelled';
                cy.getEmailDetails(emailId, expectedSubject, sentFrom)
                cy.get('@emailDetails').then((emailDetails) => {
                    let body = emailDetails.body.toLowerCase()
                    expect(emailDetails.subject).to.be.equal(expectedSubject)
                    ...
                    //Refund amount
                    let oosRefundText = 'Your credit/debit card used for payment will be refunded with the amount of $' + (Number(Number.parseFloat(Number(testData.orderTotal) - Number(20)).toFixed(2))) + '. Your refund may take 3 to 5 business days depending on your financial institution’s processing time.'
                    console.log('oosRefundText: ' + oosRefundText)
                    expect(emailDetails.body).to.include(oosRefundText)
                    let oosRefundText2 = 'A store credit of $20.00 has been added to your account. Please enter the below coupon code(s) at checkout to redeem, the next time you shop with Woolworths Online and Everyday Market from Woolworths. Minimum spend of $30 applies.'
