...
import { verifyShipmentStatusDetails } from '../../../../../support/everydayMarket/api/commands/commonHelpers'
...
Cypress.Commands.add('verifyAftershipShipmentStatusEmailDetails', (id,trackNo,sellerName,carrierName) => {
  ...
  cy.getOrderStatus(testData.orderId).as('oosOqsResponse').then((oosOqsResponse) => {
        ...
        switch (id) {
          ...
          if (id.indexOf('AvailableForPickup') == -1 ) {
expect(emailDetails.body).to.include('Estimated arrival')
}
