    ...
    cy.get('@oosEvents').then((oosEvents) => {
      cy.getOrderStatus(testData.orderId)
        ...
        .then((oosOqsResponse) => {
          ...
          const expectedSubject =
            'Everyday Market order ' + edmOrderId + ' has been cancelled'
          cy.getEmailDetails(emailId, expectedSubject, sentFrom)
          cy.get('@emailDetails').then((emailDetails) => {
            const body = emailDetails.body.toLowerCase()
            expect(emailDetails.subject).to.be.equal(expectedSubject)
            ...
            expect(emailDetails.body).to.include(
              'Sorry your order has been cancelled'
            )
            expect(emailDetails.body).to.include('Shipping Fee')
            expect(emailDetails.body).to.include(
              'We’re sorry for any inconvenience caused. Thank you for understanding.'
            )
            ...
            const oosRefundText2 =
              'A store credit of $20.00 has been added to your account. Please enter the below coupon code(s) at checkout to redeem, the next time you shop with Woolworths Online and Everyday Market from Woolworths. Minimum spend of $30 applies.'
            ...
            edmItems.forEach(function (item, i) {
              ...
              products.forEach(function (product, j) {
                ...
                expect(body).to.include(
                  Number(oosOqsResponse.MarketOrders[i].Products[j].Quantity)
                )
              })
            })
          })
