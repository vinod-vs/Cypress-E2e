...
TestFilter(['EDM', 'API', 'EDM-E2E-API', 'EmailNotifications'], () => {
  describe('[API] RP-5568 - Email Notifications | Verify delivery confirmation, partial dispatch, customer return-refund and partial OOS email notifications with split payments', () => {
    ...
    it('[API] RP-5568 - Email Notifications | Verify delivery confirmation, partial dispatch, customer return-refund and partial OOS email notifications with split payments', () => {
      ...
      cy.loginAndPlaceRequiredOrderFromTestdata(shopper, testData).then(
        (response) => {
      ...
      // Verify the order confirmation email
      cy.verifyDeliveryConfirmationEmailDetails(testData).then(
        (deliveryConfirmationEmail) => {
          ...
          cy.partialDispatchOfLineItemsInInvoice(
            ...
            ).to.be.equal(trackingId1)
            expect(response.data.attributes.shipment_carrier).to.be.equal(
              testData.carrier
            )
            expect(Number(response.data.relationships.invoice.id)).to.be.equal(
              testData.edmInvoiceId
            )
            expect(
              Number(response.data.relationships.line_items[0].id)
            ).to.be.equal(lineItemLegacyId)
            expect(Number(response.included[0].id)).to.be.equal(
              lineItemLegacyId
            )
            expect(
              Number(response.included[0].attributes.invoice_id)
            ).to.be.equal(testData.edmInvoiceId)
            expect(
              Number(response.included[1].attributes.line_item_id)
            ).to.be.equal(lineItemLegacyId)
          ...
          // Verify the customer return confirmation email
          cy.wait(5000)
          cy.verifyCustomerReturnAndRefundEmailDetails(testData).then(
            (customerReturnAndRefundEmailVerification) => {
              // OOS one quatity
              // Verify the projection and events
                ...
                cy.orderEventsApiWithRetry(orderReference, {
                  function: function (response) {
                    if (
                      !response.body.data.some(
                        (element) =>
                          element.domainEvent === 'RefundRequestUpdate'
                      ) ||
                      !response.body.data.some(
...
Cypress.Commands.add('verifyDeliveryConfirmationEmailDetails', (testData) => {
  cy.get('@orderPlacedProjection').then((orderPlacedProjection) => {
    cy.get('@orderPlacedEvents').then((orderPlacedEvents) => {
      cy.getOrderStatus(testData.orderId)
        .as('orderPlacedOqsResponse')
        .then((orderPlacedOqsResponse) => {
          const expectedSubject =
            'Delivery order #' + orderId + ' - Confirmation'
          ...
          cy.get('@emailDetails').then((emailDetails) => {
            ...
            expect(body).to.include(
              orderPlacedOqsResponse.MarketDeliveryStreet1.toLowerCase()
            )
            expect(body).to.include(
              orderPlacedOqsResponse.MarketDeliveryStreet2.toLowerCase()
            )
            expect(body).to.include(
              orderPlacedOqsResponse.MarketDeliverySuburb.toLowerCase()
            )
            expect(body).to.include(
              orderPlacedOqsResponse.MarketDeliveryPostCode.toLowerCase()
            )
            expect(body).to.include(
              orderPlacedOqsResponse.DeliveryStreet1.toLowerCase()
            )
            ...
            expect(body).to.include(
              orderPlacedOqsResponse.DeliverySuburb.toLowerCase()
            )
            expect(body).to.include(
              orderPlacedOqsResponse.DeliveryPostCode.toLowerCase()
            )
            // expect(body).to.include(orderPlacedOqsResponse.Total) not appearing in email
            expect(body).to.include(
              orderPlacedOqsResponse.Shopper.FirstName.toLowerCase()
            )
            ...
            expect(body).to.include(
              Number(Number.parseFloat(Number(testData.orderTotal)).toFixed(2))
            )
            expect(body).to.include(
              Number(
                Number.parseFloat(Number(testData.edmDeliveryCharges)).toFixed(
                  2
                )
              )
            )
            expect(body).to.include(
              Number(
                Number.parseFloat(Number(testData.wowDeliveryCharges)).toFixed(
                  2
                )
              )
            )
            expect(body).to.include(
              Number(
                Number.parseFloat(Number(testData.packagingFee)).toFixed(2)
              )
            )

            // Verify links
            expect(
              emailDetails.dom.window.document.querySelector(
                "a[href*='/shop/myaccount/myorders']"
              ) !== null
            ).to.be.true
            //expect(emailDetails.dom.window.document.querySelector("a[href*='/shop/myaccount/myinvoices']") !== null).to.be.true
            expect(
              emailDetails.dom.window.document.querySelector(
                "a[href*='/shop/myaccount/myorders/" + orderId + "']"
              ) !== null
            ).to.be.true
            expect(
              emailDetails.dom.window.document.querySelector(
                "a[href*='/shop/myaccount/trackmyorder?ordernumber=" +
                  orderId +
                  "']"
              ) !== null
            ).to.be.true
            expect(
              emailDetails.dom.window.document.querySelector(
                "a[href*='/shop/discover/shopping-online/delivery']"
              ) !== null
            ).to.be.true
            expect(
              emailDetails.dom.window.document.querySelector(
                "a[href*='/shop/page/about-us/terms-and-conditions']"
              ) !== null
            ).to.be.true

            // Verify payment method
            expect(body).to.include(
              'Paid with Everyday Rewards dollars'.toLowerCase()
            )
            ...
            expect(body).to.include(
              Number(
                Number.parseFloat(
                  Number(testData.orderTotal) - Number(20)
                ).toFixed(2)
              )
            )
            ...
            wowItems.forEach(function (item, k) {
              expect(body).to.include(
                orderPlacedOqsResponse.OrderProducts[
                  k
                ].Ordered.Name.toLowerCase()
              )
              expect(body).to.include(
                orderPlacedOqsResponse.OrderProducts[k].Ordered.Quantity
              )
              expect(body).to.include(
                orderPlacedOqsResponse.OrderProducts[k].Ordered.Total
              )
              expect(body).to.include(
                orderPlacedOqsResponse.OrderProducts[k].Ordered.SalePrice.Value
              )
            })
            ...
            edmItems.forEach(function (item, i) {
              expect(body).to.include(
                orderPlacedOqsResponse.MarketOrders[i].SellerName.toLowerCase()
              )
              expect(body).to.include(
                orderPlacedOqsResponse.MarketOrders[i].Total
              )
              expect(body).to.include(
                orderPlacedOqsResponse.MarketOrders[i].MarketShippingFee
              )
              expect(body).to.include(
                orderPlacedOqsResponse.MarketOrders[i]
                  .MarketShippingFeeBeforeDiscount
              )
              //TO FIX - Broken due to EDD change
              /*
              expect(body).to.include(
                orderPlacedOqsResponse.MarketOrders[
                  i
                ].DeliveryInfo.DayRangeDispatchNote.toLowerCase()
              )
              */
              ...
              products.forEach(function (product, j) {
                expect(body).to.include(
                  orderPlacedOqsResponse.MarketOrders[i].Products[
                    j
                  ].SellerName.toLowerCase()
                )
                expect(body).to.include(
                  orderPlacedOqsResponse.MarketOrders[i].Products[j].DeliveryFee
                )
                expect(body).to.include(
                  orderPlacedOqsResponse.MarketOrders[i].Products[
                    j
                ...
                expect(body).to.include(
                  orderPlacedOqsResponse.MarketOrders[i].Products[j].Quantity
                )
                expect(body).to.include(
                  orderPlacedOqsResponse.MarketOrders[i].Products[j].Total
                )
                expect(body).to.include(
                  orderPlacedOqsResponse.MarketOrders[i].Products[j].SalePrice
                )
              })
            })
          })
...
Cypress.Commands.add('verifyOrderOnItsWayEmailDetails', (testData) => {
  cy.get('@partialDispatchProjection').then((partialDispatchProjection) => {
    cy.get('@partialDispatchEvents').then((partialDispatchEvents) => {
      cy.getOrderStatus(testData.orderId)
        .as('partialDispatchOqsResponse')
        .then((partialDispatchOqsResponse) => {
          const expectedSubject =
            'Your Everyday Market order ' + edmOrderId + ' is on its way'
          cy.getEmailDetails(emailId, expectedSubject, sentFrom)
          cy.get('@emailDetails').then((emailDetails) => {
            const body = emailDetails.body.toLowerCase()
            expect(emailDetails.subject).to.be.equal(expectedSubject)
            ...
            expect(body).to.include(
              partialDispatchOqsResponse.Shopper.FirstName.toLowerCase()
            )

            // Verify links
            expect(
              emailDetails.dom.window.document.querySelector(
                "a[href*='/shop/page/help-and-support-faq']"
              ) !== null
            ).to.be.true
            expect(
              emailDetails.dom.window.document.querySelector(
                "a[href*='/Shop/Discover/about-us/terms-and-conditions']"
              ) !== null
            ).to.be.true
            ...
            edmItems.forEach(function (item, i) {
              expect(body).to.include(
                partialDispatchOqsResponse.MarketOrders[
                  i
                ].SellerName.toLowerCase()
              )
              expect(body).to.include(
              ...
              expect(
                emailDetails.dom.window.document.querySelector(
                  "a[href*='" +
                    partialDispatchOqsResponse.MarketOrders[i].Shipment
                      .TrackingLink +
                    "']"
                ) !== null
              ).to.be.true
              ...
              let j
              const products =
                partialDispatchOqsResponse.MarketOrders[i].Products
              products.forEach(function (product, j) {
                expect(body).to.include(
                  partialDispatchOqsResponse.MarketOrders[i].Products[
                    j
...
Cypress.Commands.add(
  'verifyCustomerReturnAndRefundEmailDetails',
  (testData) => {
    cy.get('@customerReturnProjection').then((customerReturnProjection) => {
      cy.get('@customerReturnEvents').then((customerReturnEvents) => {
        cy.getOrderStatus(testData.orderId)
          .as('customerReturnOqsResponse')
          .then((customerReturnOqsResponse) => {
            const expectedSubject =
              'Everyday Market order ' + edmOrderId + ' return created'
            cy.getEmailDetails(emailId, expectedSubject, sentFrom)
            cy.get('@emailDetails').then((emailDetails) => {
              const body = emailDetails.body.toLowerCase()
              expect(emailDetails.subject).to.be.equal(expectedSubject)
              ...
              expect(
                emailDetails.dom.window.document.querySelector(
                  "a[href*='" +
                    customerReturnOqsResponse.MarketOrders[0].Returns[0]
                      .Labels[0].Url +
                    "']"
                ) !== null
              ).to.be.true
              expect(
                emailDetails.dom.window.document.querySelector(
                  "a[href*='https://auspost.com.au/locate/']"
                ) !== null
              ).to.be.true
              expect(
                emailDetails.dom.window.document.querySelector(
              ...
              edmItems.forEach(function (item, i) {
                let j
                const products =
                  customerReturnOqsResponse.MarketOrders[i].Products
                products.forEach(function (product, j) {
                  expect(body).to.include(
                    customerReturnOqsResponse.MarketOrders[i].Products[
                      j
                  ...
                  expect(body).to.include(
                    customerReturnOqsResponse.MarketOrders[i].Products[j]
                      .SalePrice
                  )
                })
              })
            })
            const expectedSubject2 =
              'Everyday Market Refund Notification - ' + edmOrderId
            ...
            cy.get('@emailDetails').then((emailDetails) => {
              ...
              // Verify common details
              expect(body).to.include(
                customerReturnOqsResponse.Shopper.FirstName.toLowerCase()
              )
              ...
              // Refund amount
              const refundText =
                'Your credit/debit card used for payment will be refunded with the amount of $' +
                customerReturnOqsResponse.MarketOrders[0].Products[0]
                  .SalePrice +
                '. Your refund may take 3 to 5 business days depending on your financial institution’s processing time.'
    ...
    cy.get('@oosEvents').then((oosEvents) => {
      cy.getOrderStatus(testData.orderId)
        ...
        .then((oosOqsResponse) => {
          ...
          const expectedSubject =
            'Everyday Market items removed from order ' + edmOrderId
          cy.getEmailDetails(emailId, expectedSubject, sentFrom)
          cy.get('@emailDetails').then((emailDetails) => {
            const body = emailDetails.body.toLowerCase()
            expect(emailDetails.subject).to.be.equal(expectedSubject)
            ...
            expect(body).to.include(
              oosOqsResponse.Shopper.FirstName.toLowerCase()
            )

            // Refund amount
            const oosRefundText =
              'Your credit/debit card used for payment will be refunded with the amount of $' +
              oosOqsResponse.MarketOrders[0].Products[0].SalePrice +
              '. Your refund may take 3 to 5 business days depending on your financial institution’s processing time.'
            ...
            edmItems.forEach(function (item, i) {
              ...
              products.forEach(function (product, j) {
                expect(body).to.include(
                  oosOqsResponse.MarketOrders[i].Products[j].Name.toLowerCase()
                )
                expect(body).to.include('1')
                expect(body).to.include(
                  oosOqsResponse.MarketOrders[i].Products[j].SalePrice
                )
              })
            })
          })
