...
TestFilter(['B2C-API', 'EDM-API'], () => {
  describe('[API] RP-5037 | EM | MPer | Partial Customer Return Everyday Market order', () => {
    before(() => {
      cy.clearCookies({ domain: null })
      cy.clearLocalStorage({ domain: null })
    })

    it('RP-5037 | EM | MPer | Partial Customer Return Everyday Market order', () => {
        ...
        cy.ordersApiByShopperIdAndTraderOrderIdWithRetry(shopperId, orderId, {
          ...
          cy.fullDispatchAnInvoice(testData.edmInvoiceId, testData.trackingNumber, testData.carrier, testData.sellerName).then((response) => {
            ...
            For Full Return - const returnRequestMultipleLineItem = [{stockCode:testData.items[0].stockCode, quantity:testData.items[0].quantity-1, amount:testData.items[0].pricePerItem, reason:"Item is faulty", weight:1, notes:"Customer Return from EM Test Automation_Partial_Return" },
              ...
              const returnRequestLineItem = [{ stockCode: testData.items[0].stockCode, quantity: testData.items[0].quantity - 1, amount: testData.items[0].pricePerItem, reason: 'Item is faulty', weight: 12, notes: 'Customer Return from EM Test Automation_Partial_Return' }]

              cy.customerReturn(testData.edmOrderId, testData.orderReference, returnRequestLineItem).then((customerReturnResponse) => {
                expect(customerReturnResponse.invoiceId).to.be.equal(testData.edmOrderId)
                expect(customerReturnResponse.lineItems[0].total).to.be.equal((testData.items[0].quantity - 1) * testData.items[0].pricePerItem)
                expect(customerReturnResponse.lineItems[0].stockCode).to.be.equal(testData.items[0].stockCode)
                expect(customerReturnResponse.lineItems[0].quantity).to.be.equal(testData.items[0].quantity - 1)
                ...
                cy.ordersApiByShopperIdAndTraderOrderIdWithRetry(shopperId, orderId, {
                  function: function (response) {
                    if (response.body.invoices[0].refunds[0].status !== 'ReturnInitiated') {
                      cy.log('EM LineItem Status after hitting Return API was= ' + response.body.invoices[0].refunds[0].status + ' instead of "ReturnInitiated"')
                      throw new Error('EM LineItem Status after hitting Return API was= ' + response.body.invoices[0].refunds[0].status + ' instead of "ReturnInitiated"')
                    } else {
                  ...
                  cy.log('"Returned" EM LineItem Status in Order Api Projection after hitting Return API was= ' + orderApiProjectionResponse.invoices[0].refunds[0].status) // ReturnInitiated
                  expect(orderApiProjectionResponse.invoices[0].refunds[0].status).to.be.equal('ReturnInitiated')

                  // Call "refundRequestReturn" GraphQL API to Perform the "Process Return" Step
                  cy.refundRequestReturn(marketRefundId).then((refundRequestReturnResponse) => {
                    cy.log('"Process Return" EM line item Status in MarketPlacer GraphQL Response is= ' + refundRequestReturnResponse.data.refundRequestReturn.refundRequest.status) // Returned
                    expect(refundRequestReturnResponse.data.refundRequestReturn.refundRequest.status).to.be.equal('RETURNED')
                  })
                })
                ...
                cy.ordersApiByShopperIdAndTraderOrderIdWithRetry(shopperId, orderId, {
                  function: function (response) {
                    if (response.body.invoices[0].refunds[0].status !== 'Returned') {
                      cy.log('EM LineItem Status after hitting MP GraphQL refundRequestReturn API was= ' + response.body.invoices[0].refunds[0].status + ' instead of "Returned"')
                      throw new Error('EM LineItem Status after hitting MP GraphQL refundRequestReturn API was= ' + response.body.invoices[0].refunds[0].status + ' instead of "Returned"')
                    } else {
                ...
                cy.log('Verifying "Total Refund Amount" is same as the Response of getRefundDetails')
                cy.log('"totalMarketRefundAmount" Variable value from the customerReturnResponse is= ' + totalMarketRefundAmount)
