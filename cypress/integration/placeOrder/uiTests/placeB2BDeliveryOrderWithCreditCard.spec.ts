...
TestFilter(['B2B', 'UI', 'P0'], () => {
  describe('[UI] Place a delivery order on Woolworths at Work website using Credit Card', () => {
    beforeEach(() => {
    ...
    it('Place a delivery order on Woolworths at Work website using Credit Card', () => {
      const shopper =data.b2bOrderData1.shopper;
      const testdata =data.b2bOrderData1
      // Login
      cy.loginViaUi(shopper);
      // Select Delivery date Window
      cy.selectAvailableDeliveryDetailsOnFms();
      // Clear trolley
      cy.clearCartViaUi();
