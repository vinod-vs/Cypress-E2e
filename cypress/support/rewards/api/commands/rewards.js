...
Cypress.Commands.add(
  'getRewardsCardDetails',
  (partnerId, siteId, posId, loyaltySiteType, cardNo) => {
    ...
    requestBody.queryCardDetailsReq.transId = Number(
...
Cypress.Commands.add(
  'addRewardPoints',
  (partnerId, siteId, posId, loyaltySiteType, cardNo, numberOfPointsToAdd) => {
    ...
    requestBody.finaliseTransactionReq.transId = Number(
      Math.floor(Math.random() * 1000)
    )
    requestBody.finaliseTransactionReq.uniqueTransId = String(Date.now())
    requestBody.finaliseTransactionReq.cardNo = Number(cardNo)
    requestBody.finaliseTransactionReq.WOWRewards.earnedBonusPoints =
      Number(numberOfPointsToAdd)
    requestBody.finaliseTransactionReq.WOWRewards.redeemedOffers[0].earnedBonusPoints =
      Number(numberOfPointsToAdd)
