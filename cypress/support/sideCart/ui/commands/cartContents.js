Cypress.Commands.add('verifyCartContent', (test) => {
  ...
  items.forEach(item => {
    ...
    var priceLocator = ':nth-child(' + (noOfItems - index) + ') > .cart-item > .cart-item-product > .cart-item-details > .cart-item-cart-controls-wrapper > .cart-item-cart-controls-price > .cart-item-prices-container > shared-price > .price'
    var promoLocator = ':nth-child(' + (noOfItems - index) + ') > .cart-item > .cart-item-message > div > div.cartItemMessage-content > span:contains("You\'ve saved")'
    cy.checkIfElementExists(promoLocator).then((isThereAPromotionOnTheItem) => {
      ...
      if (isThereAPromotionOnTheItem == true) {
        cy.get(promoLocator).then(function (promoPrice) {
          var promoPriceValue = promoPrice.text().toString().split('$')[1]
          item.promoPrice = promoPriceValue
          item.hasPromo = true
          cy.log('PromoPriceValue: ' + item.promoPrice)
          var expectedAmountAfterPromo = (Number(item.pricePerItem) * Number(item.quantity)) - Number(item.promoPrice)
