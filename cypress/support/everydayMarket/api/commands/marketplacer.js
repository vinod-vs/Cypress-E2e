...
Cypress.Commands.add(
  'fullDispatchAnInvoice',
  (decodedInvoiceId, postageTrackingnumber, postageCarrier, sellerName) => {
    const apiKey = getApiKeyForSeller(sellerName)
    const endPoint = String(
      Cypress.env('marketplacerFullDispatchInvoiceEndpoint')
    ).replace('INVOICE_ID', decodedInvoiceId)
...
const getApiKeyForSeller = (sellerName) => {
  switch (sellerName) {
    ...
    case 'Big W':
      return Cypress.env('marketplacerBigWAPIKey')
    case 'Healthy Life':
...
*/
Cypress.Commands.add(
  'partialDispatchOfLineItemsInInvoice',
  (
    decodedInvoiceId,
    decodedLineAndquantityDetails,
    ...
    const endPoint = String(
      Cypress.env('marketplacerPartialDispatchInvoiceEndpoint')
    ).replace('INVOICE_ID', decodedInvoiceId)
    cy.api({
      method: 'POST',
      headers: {
...
*/
Cypress.Commands.add(
  'cancelLineItemInInvoice',
  (encodedInvoiceId, encodedLineItemId, quantity, dispatched) => {
    ...
    cy.refundRequestCreateInitiatedBy(
      ...
      quantity,
      dispatched,
      refundRequestInitiatorType.SELLER
    ).then((response) => {
      encodedRefundRequestId =
        response.data.refundRequestCreate.refundRequest.id
...
Cypress.Commands.add(
  ...
  (encodedInvoiceId, encodedLineItemId, quantity, dispatched) => {
    ...
    mutation = String(mutation).replace(
      'ENCODED_LINEITEM_ID',
      encodedLineItemId
    )
    ...
    cy.api({
      ...
      expect(
        response.body.data.refundRequestCreate.refundRequest.status
      ).to.equals('AWAITING')
      return response.body
    })
  }
...
Cypress.Commands.add('refundRequestReturn', (encodedRefundRequestId) => {
  ...
  cy.api({
    ...
    expect(response.status).to.eq(200)
    expect(
      response.body.data.refundRequestReturn.refundRequest.status
    ).to.equals('RETURNED')
...
Cypress.Commands.add(
  'refundRequestRefund',
  (encodedRefundRequestId, refundAmountCents) => {
    let mutation = String(`mutation{
...
Cypress.Commands.add(
  'customerReturn',
  (edmInvoiceId, orderReference, returnRequestLineItem) => {
...
Cypress.Commands.add(
  'refundRequestCreateInitiatedBy',
  (encodedInvoiceId, encodedLineItemId, quantity, dispatched, initiator) => {
    let mutation = String(`mutation{
        refundRequestCreate(input: 
          {
            invoiceId: "ENCODED_INVOICE_ID",
...
Cypress.Commands.add(
  ...
  (shippingId, decodedInvoiceId, postageTrackingnumber, sellerName) => {
    const apiKey = getApiKeyForSeller(sellerName)
    const requestBody = partialDispatchOfLineItemsInInvoice
    requestBody.data.attributes.shipment_tracking_number = postageTrackingnumber
    const endPoint =
      String(Cypress.env('marketplacerPartialDispatchInvoiceEndpoint')).replace(
        'INVOICE_ID',
        decodedInvoiceId
      ) + '/'
    cy.log(endPoint)
