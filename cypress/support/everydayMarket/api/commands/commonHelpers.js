...
export function verifyOrderTotals (testData, confirmOrderResponse) {
  ...
  expect(confirmOrderResponse.Order.MarketSubtotal).to.be.equal(
    Number(Number.parseFloat(Number(testData.edmTotal)).toFixed(2))
  )
  expect(confirmOrderResponse.Order.Subtotal).to.be.equal(
    Number(
      Number.parseFloat(
        Number(testData.edmTotal) + Number(testData.wowTotal)
      ).toFixed(2)
    )
  )
  expect(confirmOrderResponse.Order.TotalIncludingGst).to.be.equal(
    Number(Number.parseFloat(testData.orderTotal).toFixed(2))
  )
}
...
export function verifyRefundDetails (
  ...
  cy.getRefundDetails(traderOrderId).then((response) => {
    expect(response.Total).to.be.equal(
      Number(
        Number.parseFloat(
          Number(expectedEdmRefundTotal) + Number(expectedEdmShippingFeeRefund)
        ).toFixed(2)
      )
    )
    expect(response.Summaries.Market[0].Value).to.be.equal(
      Number(expectedEdmRefundTotal)
    )
    expect(response.Summaries.Market[1].Value).to.be.equal(
...
export function verifyCompleteRefundDetails (
  ...
  cy.getRefundDetails(traderOrderId).then((response) => {
    ...
    expect(response.Summaries.Woolworths[0].Value).to.be.equal(
      Number(expectedWOWRefundTotal)
    )
    expect(response.Summaries.Woolworths[1].Value).to.be.equal(
      Number(expectedWOWReusableBagsRefund)
    )
    expect(response.Summaries.Market[0].Value).to.be.equal(
      Number(expectedEdmRefundTotal)
...
export function verifyInvoiceDetails (invoice, testData) {
  ...
  // MP Invoice
  expect(invoice.MarketInvoices[0].InvoiceId).to.be.equal(
    String(testData.edmOrderId)
  )
  ...
  expect(invoice.MarketInvoices[1].Total).to.be.equal(
    Number(testData.edmDeliveryCharges)
  )
...
export function verifyInitialOrderDetails (response, testData, shopperId) {
  ...
  expect(response.invoices[0].lineItems[0].quantityPlaced).to.be.equal(
    Number(testData.items[0].quantity)
  )
  ...
  // Rewards Details
  expect(response.invoices[0].lineItems[0].reward.offerId).to.be.equal(
    'MARKETPOINTS'
  )
  expect(response.invoices[0].lineItems[0].reward.deferredDiscountAmount).to.not
    .be.null
  expect(response.invoices[0].lineItems[0].reward.quantity).to.be.equal(
    Number(testData.items[0].quantity)
  )
}
...
export function verifyOQSOrderStatus (
  ...
  // Invoke OQS api
  cy.getOrderStatus(traderOrderId)
    .then((oqsResponse) => {
  ...
  cy.get('@finalProjection').then((projection) => {
    cy.get('@oqsResponse').then((oqsResponse) => {
      ...
      if (testData !== null) {
        const wowItems = testData.items.filter(
          (item) => item.isEDMProduct === String(false)
        )
        ...
        if (wowItems.length !== 0) {
          ...
          wowItems.forEach(function (item, k) {
            ...
            if (
              ...
              ).to.be.equal(item.stockCode)
              expect(oqsResponse.OrderProducts[k].Ordered.Quantity).to.be.equal(
                item.quantity
              )
              expect(
                oqsResponse.OrderProducts[k].Ordered.SalePrice.Value
              ).to.be.equal(item.pricePerItem)
              expect(oqsResponse.OrderProducts[k].Ordered.Total).to.be.equal(
                Number(
                  Number.parseFloat(
                    Number(item.pricePerItem * item.quantity)
                  ).toFixed(2)
                )
              )
            }
      ...
      expect(oqsResponse.MarketOrders.length).to.be.equal(
        projection.invoices.length
      )
      ...
      oqsResponse.MarketOrders.forEach(function (item, i) {
        ...
        expect(oqsResponse.MarketOrders[i].Products.length).to.be.equal(
          projection.invoices[i].lineItems.length
        )
        oqsResponse.MarketOrders[i].Products.forEach(function (item, j) {
          expect(oqsResponse.MarketOrders[i].LegacyIdFormatted).to.be.equal(
            projection.invoices[i].legacyIdFormatted
          )
          expect(oqsResponse.MarketOrders[i].SellerName).to.be.equal(
            projection.invoices[i].seller.sellerName
          )
          // Verify order status
          if (args.length > 0 && args[0] !== true) {
            if (projection.invoices[i].wowStatus === 'SellerCancelled') {
              expect(oqsResponse.MarketOrders[i].Status).to.be.equal(
                'Cancelled'
              )
          ...
          expect(oqsResponse.MarketOrders[i].Total).to.be.equal(
            projection.invoices[i].invoiceTotal
          )
          expect(oqsResponse.MarketOrders[i].MarketShippingFee).to.be.equal(
            projection.shippingAmount
          )
          ...
          if (projection.invoices[i].shipments.length !== 0) {
            expect(oqsResponse.MarketOrders[i].Shipment.Carrier).to.be.equal(
              projection.invoices[i].shipments[0].carrier
            )
            expect(
              oqsResponse.MarketOrders[i].Shipment.TrackingLink
            ).to.be.equal(projection.invoices[i].shipments[0].trackingLink)
            expect(
              oqsResponse.MarketOrders[i].Shipment.TrackingNumber
            ).to.be.equal(projection.invoices[i].shipments[0].trackingNumber)
          } else {
            expect(projection.invoices[i].shipments).to.be.empty
            expect(oqsResponse.MarketOrders[i].Shipment.Carrier).to.be.undefined
            expect(oqsResponse.MarketOrders[i].Shipment.TrackingLink).to.be
              .undefined
            expect(oqsResponse.MarketOrders[i].Shipment.TrackingNumber).to.be
              .undefined
          }
          // Verify if there are any refunds
          if (projection.invoices[i].refunds.length !== 0) {
            oqsResponse.MarketOrders[i].Refunds.forEach(function (
              oqsRefund,
              m
            ) {
              expect(oqsRefund.Id).to.be.equal(
                projection.invoices[i].refunds[m].id
              )
              expect(oqsRefund.RefundAmount).to.be.equal(
                projection.invoices[i].refunds[m].refundAmount
              )
              expect(oqsRefund.InitiatedBy).to.be.equal(
                projection.invoices[i].refunds[m].initiatedBy
              )
              expect(oqsRefund.UpdatedUtc).to.be.equal(
                projection.invoices[i].refunds[m].updatedUtc
              )
              // Notes
              expect(
                JSON.stringify(oqsRefund.Notes).toLowerCase()
              ).to.deep.equal(
                JSON.stringify(
                  projection.invoices[i].refunds[m].notes
                ).toLowerCase()
              )
              // RefundItems
              oqsRefund.RefundItems.forEach(function (oqsRefundItem, n) {
                expect(oqsRefundItem.Id).to.be.equal(
                  projection.invoices[i].refunds[m].refundItems[n].id
                )
                expect(oqsRefundItem.Reason).to.be.equal(
                  projection.invoices[i].refunds[m].refundItems[n].reason
                )
                expect(oqsRefundItem.Quantity).to.be.equal(
                  projection.invoices[i].refunds[m].refundItems[n].quantity
                )
                expect(oqsRefundItem.Amount).to.be.equal(
                  projection.invoices[i].refunds[m].refundItems[n].amount
                )
                expect(oqsRefundItem.Quantity).to.be.equal(
                  projection.invoices[i].refunds[m].refundItems[n].quantity
                )
                expect(oqsRefundItem.StockCode).to.be.equal(
                  projection.invoices[i].refunds[m].refundItems[n].lineItem
                    .stockCode
                )
              })
            })
          ...
          if (projection.invoices[i].returns.length !== 0) {
            oqsResponse.MarketOrders[i].Returns.forEach(function (
              oqsReturn,
              m
            ) {
              expect(oqsReturn.ReturnId).to.be.equal(
                projection.invoices[i].returns[m].returnId
              )
              expect(oqsReturn.MarketRefundId).to.be.equal(
                projection.invoices[i].returns[m].marketRefundId
              )
              expect(oqsReturn.RefundAmount).to.be.equal(
                projection.invoices[i].returns[m].refundAmount
              )
              expect(oqsReturn.TrackingId).to.be.equal(
                projection.invoices[i].returns[m].trackingId
              )
              expect(oqsReturn.ConsignmentId).to.be.equal(
                projection.invoices[i].returns[m].consignmentId
              )
              expect(oqsReturn.CreatedUtc).to.be.equal(
                projection.invoices[i].returns[m].createdUtc
              )
              // Labels
              oqsReturn.Labels.forEach(function (oqsReturnLabel, p) {
                expect(oqsReturnLabel.Url).to.be.equal(
                  projection.invoices[i].returns[m].labels[p].url
                )
              })
              // ReturnItems
              oqsReturn.ReturnItems.forEach(function (oqsReturnItem, n) {
                expect(oqsReturnItem.Id).to.be.equal(
                  projection.invoices[i].returns[m].returnItems[n].id
                )
                oqsReturnItem.LineItems.forEach(function (
                  oqsReturnLineItem,
                  o
                ) {
                  expect(oqsReturnLineItem.Id).to.be.equal(
                    projection.invoices[i].returns[m].returnItems[n].lineItems[
                      o
                    ].id
                  )
                  expect(oqsReturnLineItem.StockCode).to.be.equal(
                    projection.invoices[i].returns[m].returnItems[n].lineItems[
                      o
                    ].stockCode
                  )
                  expect(oqsReturnLineItem.Quantity).to.be.equal(
                    projection.invoices[i].returns[m].returnItems[n].lineItems[
                      o
                    ].quantity
                  )
                  expect(oqsReturnLineItem.Amount).to.be.equal(
                    projection.invoices[i].returns[m].returnItems[n].lineItems[
                      o
                    ].amount
                  )
                  expect(oqsReturnLineItem.Reason).to.be.equal(
                    projection.invoices[i].returns[m].returnItems[n].lineItems[
                      o
                    ].reason
                  )
                  expect(oqsReturnLineItem.Weight).to.be.equal(
                    projection.invoices[i].returns[m].returnItems[n].lineItems[
                      o
                    ].weight
                  )
                  expect(oqsReturnLineItem.Notes).to.be.equal(
                    projection.invoices[i].returns[m].returnItems[n].lineItems[
                      o
                    ].notes
                  )
                  expect(oqsReturnLineItem.TrackingId).to.be.equal(
                    projection.invoices[i].returns[m].returnItems[n].lineItems[
                      o
                    ].trackingId
                  )
                  expect(oqsReturnLineItem.ConsignmentId).to.be.equal(
                    projection.invoices[i].returns[m].returnItems[n].lineItems[
                      o
                    ].consignmentId
                  )
          ...
          // Products
          expect(oqsResponse.MarketOrders[i].Products[j].StockCode).to.be.equal(
            projection.invoices[i].lineItems[j].stockCode
          )
          expect(oqsResponse.MarketOrders[i].Products[j].Quantity).to.be.equal(
            projection.invoices[i].lineItems[j].quantity
          )
          expect(
            oqsResponse.MarketOrders[i].Products[j].RefundableQuantity
          ).to.be.equal(projection.invoices[i].lineItems[j].refundableQuantity)
          expect(oqsResponse.MarketOrders[i].Products[j].Total).to.be.equal(
            projection.invoices[i].lineItems[j].totalAmount
          )
          expect(oqsResponse.MarketOrders[i].Products[j].SalePrice).to.be.equal(
            projection.invoices[i].lineItems[j].salePrice
          )
        })
      })
    })
