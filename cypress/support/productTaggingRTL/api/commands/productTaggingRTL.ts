Cypress.Commands.add('getRtlOffers', (rtlGetOffersData) => {
  ...
  let endpoint = Cypress.env('productTaggingUat') + Cypress.env('productTaggingRtlOffersEndpoint')
    cy.api({
      method : 'POST', 
      url: endpoint,
  ...
  Cypress.Commands.add('patchRtlUnboost', (rtlPatchData) => {
    ...
    let endpoint = Cypress.env('productTaggingUat') + Cypress.env('ProductTaggingUnboostEndpoint')
      cy.api({
        method : 'PATCH', 
        url: endpoint,
        body: rtlPatchData
